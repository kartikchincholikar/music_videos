...
class AudioIPAdapterTransitions(AudioNodeBase):
	...
	def process_transitions(self, images, peaks_weights, transition_mode, transition_length, min_IPA_weight, max_IPA_weight):
		...
		switch_indices = []
		index_value = 0
		for i,peak in enumerate(peaks_binary):
			if peak == 1 and i>25: #no switching for first 25 frames
				index_value += 1
			switch_indices.append(index_value)

		print("Removing any switching which can happen in the first 25 frames")
		...
		# For each transition, compute blending weights with dynamic transition length
		for idx, change_frame in enumerate(change_frames):
			# Calculate dynamic transition length
			if len(change_frames) > 1:
				# Calculate distances to previous and next change frames
				if idx > 0:
					prev_change_distance = change_frame - change_frames[idx - 1]
				else:
					prev_change_distance = change_frame

				if idx < len(change_frames) - 1:
					next_change_distance = change_frames[idx + 1] - change_frame
				else:
					next_change_distance = total_frames - change_frame

				# Dynamically adjust transition length
				dynamic_transition_length = min(
					int(prev_change_distance * 0.5),  # Half the distance to previous change
					int(next_change_distance * 0.5),  # Half the distance to next change
					transition_length  # Original maximum transition length
				)
			else:
				# If only one change frame, use original transition length
				dynamic_transition_length = transition_length

			# Compute start and end of transition
			start = max(0, change_frame - dynamic_transition_length // 2)
			end = min(total_frames, change_frame + (dynamic_transition_length + 1) // 2)
			n = end - start - 1
